...
# To illustrate we will consider the following the following
# bivaritate data collection:
...
# Let's use R code to establish matrix X :
  ...
  # Lets find the transpose of matrix X
  t(X) -> transposeX
  transposeX
  dim(X)
  ...
  # Lets find the product of X and the transpose of X
  transposeX%*%X -> ProDuct1
  ProDuct1
  ...
  solve(ProDuct1)%*%transposeXm%*%Y


  solve(ProDuct1)%*%transposeXm%*%Y -> interceptandslope
  interceptandslope
...
# R code for matrix X
  ...
  solve(Product2)%*%transposeX%*%Y # how to find slope and intercept
...
R <- matrix(c(20,
 ...
 solve(Q)%*%R # INverse multipled by r
...
# equations
 ...
 z <-matrix(c(8,
              28), ncol = 1, byrow = TRUE)
 z

 p <- matrix(c(5,2,
              23,7), ncol = 2, byrow =TRUE)
...
# has an inverse if determinat does not equal to zero
